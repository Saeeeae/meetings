...
@contextmanager
def _session_scope():
    # Stages run for hours; sessions must stay short-lived so no connection
    # is held (and possibly severed by the server) across a stage boundary.
    db = SessionLocal()
    try:
        yield db
    finally:
...
def _set_job_step(job_id: str, step: str, status: str = "processing") -> None:
...
def _cleanup_artifacts(job_id: str, upload_path: str | None, processed_audio_path: str | None) -> None:
    try:
        if settings.cleanup_uploads_on_success and upload_path:
        ...
        logger.warning("Failed to clean up artifacts for job %s: %s", job_id, cleanup_error)
...
@celery_app.task(name="app.workers.tasks.purge_expired_artifacts")
...
def _save_result(job_id: str, **fields) -> None:
    with _session_scope() as db:
        existing_result = db.query(Result).filter(Result.job_id == job_id).one_or_none()
        if existing_result:
            for name, value in fields.items():
                setattr(existing_result, name, value)
        else:
            db.add(Result(job_id=job_id, **fields))
        db.commit()
...
def process_job(job_id: str) -> None:
    ...
    try:
        ...
        _set_job_step(job_id, "preprocessing")
        audio_path = MediaService().preprocess(upload_path, job_id)
        ...
        if lexicon_service.enabled:
            ...
            logger.info("Applied the STT correction dictionary to job %s.", job_id)

        _set_job_step(job_id, "diarization")
        speaker_segments = _run_diarization(audio_path)

        _set_job_step(job_id, "alignment")
        ...
        if settings.vllm_on_demand:
            _set_job_step(job_id, "llm_startup")
        ...
        _set_job_step(job_id, "completed", status="completed")
